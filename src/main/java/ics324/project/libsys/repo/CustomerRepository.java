...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    ...
    @Query(value = "select * from customer c where c.id in (select  br.customer_id from borrow_record br where not br.status=0 and 119<(-br.check_out_date+current_date())) and c.id in  (select  br.customer_id from borrow_record br where not br.status=0 having 2<(select count(customer_id)  from borrow_record bs where customer_id=14 and not  bs.status=0 ));",nativeQuery = true)
